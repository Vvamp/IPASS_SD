...
public class OpeningResource {
    ...
    public Response now() {
        ...
        result.setOpen(now.isAfter(openingToday) && now.isBefore(closingToday));
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("HH:mm");

        if(result.isOpen()) {
            result.setClosingTime(formatter.format(closingToday));
            result.setOpeningTime(formatter.format(getOpeningTimeForDay(now.plusDays(1))));
        }else{
            result.setOpeningTime(formatter.format(getOpeningTimeForDay(now.plusHours(12))));
            result.setClosingTime(formatter.format(getClosingTimeForDay(now.plusHours(12))));
        }
