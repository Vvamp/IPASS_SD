...
public class LoginResource {
    ...
    public Response authenticateUser(LogonRequest request) {
        try {
            ...
            String role = loginManager.validateLogin(request.getUser(), request.getPassword());
            if (role == null) return Response.status(Response.Status.UNAUTHORIZED).build();
            String token = loginManager.createToken(request.getUser(), role);
            loginManager.validateToken(token);

            LoginResult lr = new LoginResult(token, request.getUser(), role);
    ...
    public Response logoutUser(TokenRequest request) {
        LoginManager loginManager = Main.getLoginManager();
        loginManager.invalidateToken(request.getToken());
